...
public class factoriaAlgoritmos implements IntFactoriasAl {
    ...
    public RecSys Selecion(String elecion, String distance) throws IOException, FilaVacia, TablaVacia, Comparator {
        return switch (elecion) {
            case "Recommend based on songs features" -> IsKnn(distance);
            case "Recommend based on guessed genre" -> IsKmeans(distance);
            default -> throw new IllegalArgumentException("Parametro no reconocido");
        };
    }
